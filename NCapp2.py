...
if 'result_df' in st.session_state:
    ...
    b64_registered = base64.b64encode(st.session_state['result_df'].round(1).to_csv(index=False, encoding='utf-8').encode()).decode()
...
if 'result_df' in st.session_state:
    ...
    # 登録データと合計を組み合わせた表をCSVファイルとしてダウンロードするリンクを生成
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    combined_filename = f'combined_food_list_{current_time}.csv'
    b64_combined = base64.b64encode(combined_table.round(1).to_csv(index=False,encoding='utf-8').encode()).decode()
    href_combined = f'<a href="data:file/csv;base64,{b64_combined}" download="{combined_filename}">登録データと合計を組み合わせた表をダウンロード</a>'
